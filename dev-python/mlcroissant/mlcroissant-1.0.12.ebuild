# This ebuild was auto-generated by homeassistant script, and the script was not able to find a better candidate.
EAPI=8

DISTUTILS_USE_PEP517=setuptools
PYTHON_COMPAT=( python3_{12,13{,t}} )
PYPI_NO_NORMALIZE=1
GENERATED_IUSE="audio beam dev git image parquet"
IUSE="${GENERATED_IUSE}"

inherit distutils-r1 pypi

DESCRIPTION=""
HOMEPAGE="
  https://pypi.org/project/mlcroissant/"

LICENSE=""
SLOT="0"
KEYWORDS="amd64 arm64"

GENERATED_DEPEND="
	dev-python/absl-py[${PYTHON_USEDEP}]
	beam? ( dev-python/apache-beam[${PYTHON_USEDEP}] )
	dev? ( ~dev-python/black-23.11.0[${PYTHON_USEDEP}] )
	>=dev-python/etils-1.7.0[epath,${PYTHON_USEDEP}]
	dev? ( dev-python/flake8-docstrings[${PYTHON_USEDEP}] )
	git? ( dev-python/gitpython[${PYTHON_USEDEP}] )
	dev-python/jsonpath-rw[${PYTHON_USEDEP}]
	audio? ( dev-python/librosa[${PYTHON_USEDEP}] )
	dev? ( dev-python/mlcroissant[audio,${PYTHON_USEDEP}] )
	dev? ( dev-python/mlcroissant[beam,${PYTHON_USEDEP}] )
	dev? ( dev-python/mlcroissant[git,${PYTHON_USEDEP}] )
	dev? ( dev-python/mlcroissant[image,${PYTHON_USEDEP}] )
	dev? ( dev-python/mlcroissant[parquet,${PYTHON_USEDEP}] )
	dev? ( dev-python/mypy[${PYTHON_USEDEP}] )
	dev-python/networkx[${PYTHON_USEDEP}]
	dev-python/pandas[${PYTHON_USEDEP}]
	dev-python/pandas-stubs[${PYTHON_USEDEP}]
	image? ( dev-python/pillow[${PYTHON_USEDEP}] )
	parquet? ( dev-python/pyarrow[${PYTHON_USEDEP}] )
	dev? ( dev-python/pyflakes[${PYTHON_USEDEP}] )
	dev? ( dev-python/pygraphviz[${PYTHON_USEDEP}] )
	dev? ( dev-python/pytest[${PYTHON_USEDEP}] )
	dev-python/python-dateutil[${PYTHON_USEDEP}]
	dev? ( dev-python/pytype[${PYTHON_USEDEP}] )
	dev-python/rdflib[${PYTHON_USEDEP}]
	dev-python/requests[${PYTHON_USEDEP}]
	audio? ( ~dev-python/soxr-0.4.0_beta1[${PYTHON_USEDEP}] )
	dev? ( dev-python/torchdata[${PYTHON_USEDEP}] )
	dev-python/tqdm[${PYTHON_USEDEP}]
"
RDEPEND="${GENERATED_DEPEND}"

distutils_enable_tests pytest