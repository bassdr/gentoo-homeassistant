# This ebuild was auto-generated by homeassistant script, and the script was not able to find a better candidate.
EAPI=8

DISTUTILS_USE_PEP517=setuptools
PYTHON_COMPAT=( python3_{12,13{,t}} )
PYPI_NO_NORMALIZE=1
GENERATED_IUSE="color flynt isort release test"
IUSE="${GENERATED_IUSE}"

inherit distutils-r1 pypi

DESCRIPTION=""
HOMEPAGE="
  https://pypi.org/project/darker/"

LICENSE="BSD"
SLOT="0"
KEYWORDS="amd64 arm64"

GENERATED_DEPEND="
	test? ( >=dev-python/airium-0.2.3[${PYTHON_USEDEP}] )
	>=dev-python/black-22.3.0[${PYTHON_USEDEP}]
	test? ( >=dev-python/black-22.3.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/cryptography-3.3.2[${PYTHON_USEDEP}] )
	release? ( ~dev-python/darkgray-dev-tools-0.0.2[${PYTHON_USEDEP}] )
	~dev-python/darkgraylib-1.2.0[${PYTHON_USEDEP}]
	test? ( >=dev-python/defusedxml-0.7.1[${PYTHON_USEDEP}] )
	flynt? ( >=dev-python/flynt-0.76[${PYTHON_USEDEP}] )
	test? ( >=dev-python/flynt-0.76[${PYTHON_USEDEP}] )
	~dev-python/graylint-1.1.1[${PYTHON_USEDEP}]
	isort? ( >=dev-python/isort-5.0.1[${PYTHON_USEDEP}] )
	test? ( >=dev-python/isort-5.0.1[${PYTHON_USEDEP}] )
	test? ( >=dev-python/mypy-0.990[${PYTHON_USEDEP}] )
	test? ( dev-python/pathspec[${PYTHON_USEDEP}] )
	test? ( dev-python/pip-requirements-parser[${PYTHON_USEDEP}] )
	test? ( dev-python/pydocstyle[${PYTHON_USEDEP}] )
	color? ( >=dev-python/pygments-2.4.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pygments[${PYTHON_USEDEP}] )
	test? ( dev-python/pylint[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-6.2.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-kwparametrize-0.0.3[${PYTHON_USEDEP}] )
	test? ( >=dev-python/regex-2021.4.4[${PYTHON_USEDEP}] )
	test? ( >=dev-python/requests-cache-0.7[${PYTHON_USEDEP}] )
	test? ( >=dev-python/ruamel-yaml-0.17.21[${PYTHON_USEDEP}] )
	test? ( >=dev-python/ruff-0.0.292[${PYTHON_USEDEP}] )
	>=dev-python/toml-0.10.0[${PYTHON_USEDEP}]
	test? ( >=dev-python/twine-2.0.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/types-requests-2.27.9[${PYTHON_USEDEP}] )
	test? ( >=dev-python/types-toml-0.10.4[${PYTHON_USEDEP}] )
	test? ( >=dev-python/urllib3-1.25.9[${PYTHON_USEDEP}] )
	test? ( >=dev-python/wheel-0.21.0[${PYTHON_USEDEP}] )
"
RDEPEND="${GENERATED_DEPEND}"

distutils_enable_tests pytest