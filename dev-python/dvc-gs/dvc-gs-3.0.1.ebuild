# This ebuild was auto-generated by homeassistant script, and the script was not able to find a better candidate.
EAPI=8

DISTUTILS_USE_PEP517=setuptools
PYTHON_COMPAT=( python3_{12,13{,t}} )
PYPI_NO_NORMALIZE=1
GENERATED_IUSE="tests"
IUSE="${GENERATED_IUSE}"

inherit distutils-r1 pypi

DESCRIPTION=""
HOMEPAGE="
  https://pypi.org/project/dvc-gs/"

LICENSE=""
SLOT="0"
KEYWORDS="amd64 arm64"

GENERATED_DEPEND="${PYTHON_DEPS}
	tests? ( ~dev-python/collective-checkdocs-0.2[${PYTHON_USEDEP}] )
	dev-python/dvc[${PYTHON_USEDEP}]
	tests? ( dev-python/dvc[testing,${PYTHON_USEDEP}] )
	tests? ( ~dev-python/filelock-3.3.2[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/flaky-3.7.0[${PYTHON_USEDEP}] )
	>=dev-python/gcsfs-2022.11.0[${PYTHON_USEDEP}]
	tests? ( >=dev-python/google-cloud-storage-2.7.0[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/mock-4.0.3[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/mypy-0.931[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pydocstyle-6.1.1[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pygments-2.10.0[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pylint-2.15.9[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pylint-plugin-utils-0.6[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-7.1.2[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-cov-3.0.0[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-lazy-fixture-0.6.3[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-mock-3.6.1[${PYTHON_USEDEP}] )
	tests? ( >=dev-python/pytest-servers-0.3.0[gcs,${PYTHON_USEDEP}] )
	tests? ( ~dev-python/pytest-xdist-2.4.0[${PYTHON_USEDEP}] )
	>=dev-python/pywin32-225[${PYTHON_USEDEP}]
	tests? ( ~dev-python/types-requests-2.25.11[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/types-tabulate-0.8.3[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/types-toml-0.10.1[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/wget-3.2[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/wheel-0.37.0[${PYTHON_USEDEP}] )
	tests? ( ~dev-python/xmltodict-0.12.0[${PYTHON_USEDEP}] )
"
RDEPEND="${GENERATED_DEPEND}"

distutils_enable_tests pytest
