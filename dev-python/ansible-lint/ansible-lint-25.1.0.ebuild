# This ebuild was auto-generated by homeassistant script, and the script was not able to find a better candidate.
EAPI=8

DISTUTILS_USE_PEP517=setuptools
PYTHON_COMPAT=( python3_{12,13{,t}} )
GENERATED_IUSE="docs test"
IUSE="${GENERATED_IUSE}"

inherit distutils-r1 pypi

DESCRIPTION=""
HOMEPAGE="
  https://pypi.org/project/ansible-lint/"

LICENSE=""
SLOT="0"
KEYWORDS="amd64 arm64"

GENERATED_DEPEND="
	>=dev-python/ansible-compat-24.10.0[${PYTHON_USEDEP}]
	>=dev-python/ansible-core-2.16.0[${PYTHON_USEDEP}]
	>=dev-python/black-24.3.0[${PYTHON_USEDEP}]
	test? ( dev-python/black[${PYTHON_USEDEP}] )
	test? ( >=dev-python/coverage-6.4.4[toml,${PYTHON_USEDEP}] )
	test? ( dev-python/coverage-enable-subprocess[${PYTHON_USEDEP}] )
	>=dev-python/filelock-3.8.2[${PYTHON_USEDEP}]
	dev-python/importlib-metadata[${PYTHON_USEDEP}]
	test? ( dev-python/jmespath[${PYTHON_USEDEP}] )
	>=dev-python/jsonschema-4.10.0[${PYTHON_USEDEP}]
	test? ( >=dev-python/license-expression-30.3.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/mkdocs-ansible-24.12.0[${PYTHON_USEDEP}] )
	test? ( dev-python/mypy[${PYTHON_USEDEP}] )
	test? ( dev-python/netaddr[${PYTHON_USEDEP}] )
	>=dev-python/packaging-22.0[${PYTHON_USEDEP}]
	>=dev-python/pathspec-0.10.3[${PYTHON_USEDEP}]
	test? ( dev-python/pip[${PYTHON_USEDEP}] )
	test? ( dev-python/psutil[${PYTHON_USEDEP}] )
	test? ( dev-python/pylint[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-7.2.2[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-instafail-0.5.0[${PYTHON_USEDEP}] )
	test? ( dev-python/pytest-mock[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-plus-0.6[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-xdist-2.1.0[psutil,setproctitle,${PYTHON_USEDEP}] )
	>=dev-python/pyyaml-6.0.2[${PYTHON_USEDEP}]
	!=dev-python/ruamel-yaml-0.18.7[${PYTHON_USEDEP}]
	test? ( >=dev-python/ruamel-yaml-0.17.31[${PYTHON_USEDEP}] )
	test? ( dev-python/ruamel-yaml-clib[${PYTHON_USEDEP}] )
	>=dev-python/subprocess-tee-0.4.1[${PYTHON_USEDEP}]
	test? ( >=dev-python/tox-4.0.0[${PYTHON_USEDEP}] )
	test? ( dev-python/types-jsonschema[${PYTHON_USEDEP}] )
	test? ( dev-python/types-pyyaml[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/uv-0.5.5[${PYTHON_USEDEP}] )
	$(python_gen_cond_dep '>=dev-python/wcmatch-8.1.2[${PYTHON_USEDEP}]' python3_12)
	$(python_gen_cond_dep '>=dev-python/wcmatch-8.5.0[${PYTHON_USEDEP}]' python3_13{,t})
	>=dev-python/yamllint-1.34.0[${PYTHON_USEDEP}]
"
RDEPEND="${GENERATED_DEPEND}"

distutils_enable_tests pytest
