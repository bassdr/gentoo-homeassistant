# This ebuild was auto-generated by homeassistant script, and the script was not able to find a better candidate.
EAPI=8

DISTUTILS_USE_PEP517=setuptools
PYTHON_COMPAT=( python3_{12,13{,t}} )
PYPI_NO_NORMALIZE=1
GENERATED_IUSE="all azure black dev docs gcs github hdfs s3 test"
IUSE="${GENERATED_IUSE}"

inherit distutils-r1 pypi

DESCRIPTION=""
HOMEPAGE="
  https://pypi.org/project/papermill/"

LICENSE="BSD"
SLOT="0"
KEYWORDS="amd64 arm64"

GENERATED_DEPEND="
	$(python_gen_cond_dep '>=dev-python/aiohttp-3.9.0[${PYTHON_USEDEP}]' python3_12)
	dev-python/ansicolors[${PYTHON_USEDEP}]
	dev? ( >=dev-python/attrs-17.4.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/attrs-17.4.0[${PYTHON_USEDEP}] )
	all? ( >=dev-python/azure-datalake-store-0.0.30[${PYTHON_USEDEP}] )
	azure? ( >=dev-python/azure-datalake-store-0.0.30[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/azure-datalake-store-0.0.30[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/azure-datalake-store-0.0.30[${PYTHON_USEDEP}] )
	test? ( >=dev-python/azure-datalake-store-0.0.30[${PYTHON_USEDEP}] )
	all? ( >=dev-python/azure-identity-1.3.1[${PYTHON_USEDEP}] )
	azure? ( >=dev-python/azure-identity-1.3.1[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/azure-identity-1.3.1[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/azure-identity-1.3.1[${PYTHON_USEDEP}] )
	test? ( >=dev-python/azure-identity-1.3.1[${PYTHON_USEDEP}] )
	all? ( >=dev-python/azure-storage-blob-12.1.0[${PYTHON_USEDEP}] )
	azure? ( >=dev-python/azure-storage-blob-12.1.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/azure-storage-blob-12.1.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/azure-storage-blob-12.1.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/azure-storage-blob-12.1.0[${PYTHON_USEDEP}] )
	all? ( >=dev-python/black-19.3_beta0[${PYTHON_USEDEP}] )
	black? ( >=dev-python/black-19.3_beta0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/black-19.3_beta0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/black-19.3_beta0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/black-19.3_beta0[${PYTHON_USEDEP}] )
	all? ( dev-python/boto3[${PYTHON_USEDEP}] )
	dev? ( dev-python/boto3[${PYTHON_USEDEP}] )
	docs? ( dev-python/boto3[${PYTHON_USEDEP}] )
	s3? ( dev-python/boto3[${PYTHON_USEDEP}] )
	test? ( dev-python/boto3[${PYTHON_USEDEP}] )
	dev? ( dev-python/botocore[${PYTHON_USEDEP}] )
	test? ( dev-python/botocore[${PYTHON_USEDEP}] )
	dev? ( dev-python/bumpversion[${PYTHON_USEDEP}] )
	test? ( dev-python/bumpversion[${PYTHON_USEDEP}] )
	dev? ( dev-python/check-manifest[${PYTHON_USEDEP}] )
	test? ( dev-python/check-manifest[${PYTHON_USEDEP}] )
	dev-python/click[${PYTHON_USEDEP}]
	dev? ( dev-python/codecov[${PYTHON_USEDEP}] )
	test? ( dev-python/codecov[${PYTHON_USEDEP}] )
	dev? ( dev-python/coverage[${PYTHON_USEDEP}] )
	test? ( dev-python/coverage[${PYTHON_USEDEP}] )
	dev-python/entrypoints[${PYTHON_USEDEP}]
	docs? ( dev-python/entrypoints[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/furo-2023.9.10[${PYTHON_USEDEP}] )
	all? ( >=dev-python/gcsfs-0.2.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/gcsfs-0.2.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/gcsfs-0.2.0[${PYTHON_USEDEP}] )
	gcs? ( >=dev-python/gcsfs-0.2.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/gcsfs-0.2.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/google-compute-engine[${PYTHON_USEDEP}] )
	test? ( dev-python/google-compute-engine[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/ipython-5.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/ipython-5.0[${PYTHON_USEDEP}] )
	dev? ( dev-python/ipywidgets[${PYTHON_USEDEP}] )
	test? ( dev-python/ipywidgets[${PYTHON_USEDEP}] )
	dev? ( <dev-python/moto-5.1.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/moto-4.2.8[${PYTHON_USEDEP}] )
	test? ( <dev-python/moto-5.1.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/myst-parser-2.0.0[${PYTHON_USEDEP}] )
	>=dev-python/nbclient-0.2.0[${PYTHON_USEDEP}]
	>=dev-python/nbformat-5.2.0[${PYTHON_USEDEP}]
	docs? ( dev-python/nbformat[${PYTHON_USEDEP}] )
	dev? ( dev-python/notebook[${PYTHON_USEDEP}] )
	test? ( dev-python/notebook[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pip-18.1[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pip-18.1[${PYTHON_USEDEP}] )
	all? ( >=dev-python/pyarrow-2.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/pyarrow-2.0[${PYTHON_USEDEP}] )
	hdfs? ( >=dev-python/pyarrow-2.0[${PYTHON_USEDEP}] )
	all? ( >=dev-python/pygithub-1.55[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/pygithub-1.55[${PYTHON_USEDEP}] )
	github? ( >=dev-python/pygithub-1.55[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-4.1[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-4.1[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-cov-2.6.1[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-cov-2.6.1[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-env-0.6.2[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-env-0.6.2[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/pytest-mock-1.10[${PYTHON_USEDEP}] )
	test? ( >=dev-python/pytest-mock-1.10[${PYTHON_USEDEP}] )
	dev-python/pyyaml[${PYTHON_USEDEP}]
	dev? ( dev-python/recommonmark[${PYTHON_USEDEP}] )
	test? ( dev-python/recommonmark[${PYTHON_USEDEP}] )
	all? ( >=dev-python/requests-2.21.0[${PYTHON_USEDEP}] )
	azure? ( >=dev-python/requests-2.21.0[${PYTHON_USEDEP}] )
	dev-python/requests[${PYTHON_USEDEP}]
	dev? ( >=dev-python/requests-2.21.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/requests-2.21.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/requests-2.21.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/setuptools-38.6.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/setuptools-38.6.0[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-7.2.6[${PYTHON_USEDEP}] )
	docs? ( >=dev-python/sphinx-copybutton-0.5.2[${PYTHON_USEDEP}] )
	>=dev-python/tenacity-5.0.2[${PYTHON_USEDEP}]
	dev? ( dev-python/tox[${PYTHON_USEDEP}] )
	test? ( dev-python/tox[${PYTHON_USEDEP}] )
	>=dev-python/tqdm-4.32.2[${PYTHON_USEDEP}]
	dev? ( >=dev-python/twine-1.11.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/twine-1.11.0[${PYTHON_USEDEP}] )
	dev? ( >=dev-python/wheel-0.31.0[${PYTHON_USEDEP}] )
	test? ( >=dev-python/wheel-0.31.0[${PYTHON_USEDEP}] )
	dev? ( dev-vcs/pre-commit[${PYTHON_USEDEP}] )
	test? ( dev-vcs/pre-commit[${PYTHON_USEDEP}] )
"
RDEPEND="${GENERATED_DEPEND}"

distutils_enable_tests pytest